...
void OBSApp::InitUserConfigDefaults()
{
	...
	/* Pulse Weaver is a single native studio window.  Never let closing that
	 * window silently leave the isolated engine running in the notification
	 * area.  These are product invariants rather than migration defaults, so
	 * stale portable profiles cannot turn tray persistence back on. */
	config_set_bool(userConfig, "BasicWindow", "SysTrayEnabled", false);
	config_set_bool(userConfig, "BasicWindow", "SysTrayWhenStarted", false);
	config_set_bool(userConfig, "BasicWindow", "SysTrayMinimizeToTray", false);
