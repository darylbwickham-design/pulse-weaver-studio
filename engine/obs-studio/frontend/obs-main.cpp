...
static constexpr char vcRunErrorMsg[] = "Pulse Weaver requires a newer version of the Microsoft Visual C++ "
...
int main(int argc, char *argv[])
{
	...
	/* Browser sources and capture helpers belong to this product process.
	 * Closing the native shell releases the entire child process tree. */
	ConfigurePulseWeaverProcessTree();
	...
	for (int i = 1; i < argc; i++) {
		if (arg_is(argv[i], "--multi", "-m")) {
			...
			std::cout << "Pulse Weaver Core - " << App()->GetVersionString(false) << "\n";
			exit(0);
		}
	}
