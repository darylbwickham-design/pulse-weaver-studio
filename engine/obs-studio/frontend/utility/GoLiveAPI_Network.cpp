...
void HandleGoLiveApiErrors(QWidget *parent, const json &raw_json, const GoLiveApi::Config &config)
{
	...
	auto warn_continue = [&](QString message) {
		...
		QMetaObject::invokeMethod(
			parent,
			[=] {
				if (parent->property("pulseWeaverAutoAcceptEnhancedBroadcastingWarning").toBool()) {
					blog(LOG_INFO,
					     "Pulse Weaver: automatically accepted Twitch Enhanced Broadcasting recommendation warning");
					return false;
				}
