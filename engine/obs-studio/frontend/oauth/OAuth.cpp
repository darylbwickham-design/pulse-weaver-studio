...
bool OAuth::GetTokenPKCE(const char *url, const std::string &client_id, const std::string &redirect_uri,
			 int scope_ver, const std::string &auth_code, const std::string &code_verifier, bool retry)
{
	return GetTokenInternal(url, client_id, {}, redirect_uri, scope_ver, auth_code, retry, code_verifier);
}

bool OAuth::GetTokenPKCE(const char *url, const std::string &client_id, const std::string &secret,
			 const std::string &redirect_uri, int scope_ver, const std::string &auth_code,
			 const std::string &code_verifier, bool retry)
{
	return GetTokenInternal(url, client_id, secret, redirect_uri, scope_ver, auth_code, retry, code_verifier);
}
...
try {
	...
	auto encoded = [](const std::string &value) {
		return QUrl::toPercentEncoding(QString::fromUtf8(value.c_str())).toStdString();
	};

	std::string post_data;
	post_data += "action=redirect&client_id=";
	post_data += encoded(client_id);
	if (!secret.empty()) {
		post_data += "&client_secret=";
		post_data += encoded(secret);
	}
	if (!redirect_uri.empty()) {
		post_data += "&redirect_uri=";
		post_data += encoded(redirect_uri);
	}

	if (!auth_code.empty()) {
		post_data += "&grant_type=authorization_code&code=";
		post_data += encoded(auth_code);
		if (!code_verifier.empty()) {
			post_data += "&code_verifier=";
			post_data += encoded(code_verifier);
		}
	} else {
		post_data += "&grant_type=refresh_token&refresh_token=";
		post_data += encoded(refresh_token);
	}
