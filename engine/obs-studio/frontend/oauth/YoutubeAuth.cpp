...
std::shared_ptr<Auth> YoutubeAuth::Login(QWidget *owner, const std::string &service)
{
	...
	const QString codeVerifier = QString::fromLatin1(verifierBytes);
	...
	if (!auth->GetTokenPKCE(YOUTUBE_TOKEN_URL, clientid, clientSecret, QT_TO_UTF8(redirect_uri), YOUTUBE_SCOPE_VERSION,
				QT_TO_UTF8(auth_code), QT_TO_UTF8(codeVerifier), true)) {
		return nullptr;
	}
