...
static void DrawPulseWeaverCanvasEdge(float cx, float cy, float previewScale)
{
	/* Draw inside the native canvas viewport, not around the preview widget.
	 * A widget border outlines the whole editor and therefore cannot show where
	 * the 16:9 programme actually stops when the preview is zoomed or centred. */
	const float pixel = 1.0f / std::max(previewScale, 0.01f);
	gs_effect_t *effect = obs_get_base_effect(OBS_EFFECT_SOLID);
	gs_eparam_t *color = gs_effect_get_param_by_name(effect, "color");
	vec4 edgeColor;
	vec4_set(&edgeColor, 0.13f, 0.83f, 0.93f, 1.0f);
	gs_effect_set_vec4(color, &edgeColor);
	for (int pass = 0; pass < 2; ++pass) {
		const float inset = pixel * float(pass + 1);
		gs_render_start(true);
		gs_vertex2f(inset, inset); gs_vertex2f(cx - inset, inset);
		gs_vertex2f(cx - inset, inset); gs_vertex2f(cx - inset, cy - inset);
		gs_vertex2f(cx - inset, cy - inset); gs_vertex2f(inset, cy - inset);
		gs_vertex2f(inset, cy - inset); gs_vertex2f(inset, inset);
		gs_vertbuffer_t *outline = gs_render_save();
		gs_load_vertexbuffer(outline);
...
void OBSBasic::RenderMain(void *data, uint32_t, uint32_t)
{
	...
	/* This display is the Camera workspace's real OBS Studio Preview surface.
	 * Keep the programme edge visible in Studio Mode too: checking the outer
	 * Pulse Weaver page here was unreliable after OBS reparented the preview. */
	DrawPulseWeaverCanvasEdge(float(ovi.base_width), float(ovi.base_height), window->previewScale);
	gs_load_vertexbuffer(nullptr);
