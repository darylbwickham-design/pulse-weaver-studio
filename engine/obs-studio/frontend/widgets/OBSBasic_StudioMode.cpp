...
{
	/* Pulse Weaver deliberately keeps Camera in Preview/Program mode: choosing
	 * a scene is preparation, while activating a Stage is the sole programme
	 * switch. Do not let the embedded native control disable that safety rule. */
	if (pulsePages) {
...
void OBSBasic::SetPreviewProgramMode(bool enabled)
{
	...
	if (IsPreviewProgramMode()) {
		...
		/* Pulse Weaver uses Studio Mode as an output-safety boundary, not as a
		 * second editor surface.  Show Control already presents the live
		 * platform programmes.  Keep the programme scene/transition machinery
		 * active, but leave Camera as one clear selected-scene editor. */
		if (pulsePages) {
			programOptions->hide();
			programWidget->hide();
		}
...
{
	bool labels = !pulsePages && previewProgramMode
