...
OBSBasic::OBSBasic(QWidget *parent) : OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	InitPulseWeaverShell();

	/* Present Pulse Weaver immediately. OBS traditionally waits until the
	 * entire engine/profile bootstrap has completed before showing its main
	 * window, which can leave a headless-looking process during slow device
	 * discovery. The studio shell stays responsive while native services load. */
	...
	/* Pulse Weaver studio layout: scenes left, sources right, editor controls
	 * across the bottom, and the native preview at the centre. Live-output
	 * controls belong exclusively to Show Control. */
	QMainWindow *cameraDockHost = pulseCameraDockHost ? pulseCameraDockHost.data() : this;
	cameraDockHost->setDockOptions(QMainWindow::AnimatedDocks | QMainWindow::AllowNestedDocks |
				       QMainWindow::AllowTabbedDocks | QMainWindow::GroupedDragging);
	cameraDockHost->setDockNestingEnabled(true);
	for (QDockWidget *dock : QList<QDockWidget *>{ui->scenesDock, ui->sourcesDock, ui->mixerDock,
						      ui->transitionsDock, controlsDock}) {
		/* setupUi initially registers the stock docks on OBSBasic. Explicitly
		 * remove those layout entries before reparenting; addDockWidget alone
		 * moves the widget but can leave a large empty placeholder in the outer
		 * QMainWindow layout. */
		removeDockWidget(dock);
		dock->setParent(cameraDockHost);
		dock->setAllowedAreas(Qt::AllDockWidgetAreas);
	}
	ui->scenesDock->setAccessibleName("Scenes dock");
	ui->sourcesDock->setAccessibleName("Sources dock");
	ui->mixerDock->setAccessibleName("Audio Mixer dock");
	ui->transitionsDock->setAccessibleName("Scene Transitions dock");
	controlsDock->setAccessibleName("Controls dock");
	cameraDockHost->addDockWidget(Qt::LeftDockWidgetArea, ui->scenesDock);
	cameraDockHost->addDockWidget(Qt::RightDockWidgetArea, ui->sourcesDock);
	cameraDockHost->addDockWidget(Qt::BottomDockWidgetArea, ui->mixerDock);
	cameraDockHost->splitDockWidget(ui->mixerDock, ui->transitionsDock, Qt::Horizontal);
	int sideDockWidth = std::min(width() * 30 / 100, 320);
	cameraDockHost->resizeDocks({ui->scenesDock, ui->sourcesDock}, {sideDockWidth, sideDockWidth}, Qt::Horizontal);
	controlsDock->setVisible(false);
	SetPulseWeaverWorkspace(0);

	startingDockLayout = saveState();
	pulseCameraStartingDockLayout = cameraDockHost->saveState(1);
...
void OBSBasic::OBSInit()
{
	...
	RestorePulseWeaverYouTubeAccount();
	if (!ResetAudio()) {
	...
	/* Pulse Weaver's Camera is permanently Preview/Program: scene selection
...
#undef SET_VISIBILITY
	...
	/* Pulse Weaver is the primary studio shell, not a tray-hosted OBS helper.
	 * Always present the native editor when the product is launched. */
	...
	const char *dockStateStr = config_get_string(App()->GetUserConfig(), "BasicWindow", "PulseWeaverCameraDockState");
	const int pulseDockLayoutVersion =
		(int)config_get_int(App()->GetUserConfig(), "BasicWindow", "PulseWeaverDockLayoutVersion");

	/* Version 4 intentionally gives Camera its own QMainWindow dock host. Older
	 * outer-window states cannot describe those snap regions, so seed the
	 * contained workspace once and persist its state separately. */
	if (pulseDockLayoutVersion < 4) {
		on_resetDocks_triggered(true);
		config_set_int(App()->GetUserConfig(), "BasicWindow", "PulseWeaverDockLayoutVersion", 4);
	} else if (!dockStateStr) {
		on_resetDocks_triggered(true);
	} else {
		QByteArray dockState = QByteArray::fromBase64(QByteArray(dockStateStr));
		if (!pulseCameraDockHost || !pulseCameraDockHost->restoreState(dockState, 1))
			on_resetDocks_triggered(true);
	}
	...
	/* Pulse Weaver owns onboarding and ships sensible isolated defaults. The
	 * upstream OBS auto-configuration wizard is still available from Studio →
	 * Tools, but must not interrupt Pulse Weaver's first launch. */
	...
	/* Loading a scene collection restores upstream dock visibility after the
	 * shell was constructed.  Re-assert the product workspace once startup is
	 * complete and provision the independent vertical canvas on first run. */
	QTimer::singleShot(0, this, [this] {
		EnsurePulseWeaverVerticalCanvas();
		SetPulseWeaverWorkspace(pulsePages ? pulsePages->currentIndex() : 0);
	});
}
...
void OBSBasic::saveAll()
{
	...
	std::call_once(saveOnceFlag, [this]() {
		...
		if (pulseCameraDockHost)
			config_set_string(App()->GetUserConfig(), "BasicWindow", "PulseWeaverCameraDockState",
					  pulseCameraDockHost->saveState(1).toBase64().constData());
...
{
	stringstream name;
	name << "Pulse Weaver Streaming Studio";
