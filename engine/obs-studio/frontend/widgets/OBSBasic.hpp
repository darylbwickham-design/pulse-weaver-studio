...
class OBSBasic : public OBSMainWindow {
	...
	friend class PulseVerticalEditor;
...
private:
	...
	/* Pulse Weaver is the native frontend shell.  These pages live inside
	 * OBSBasic so Camera keeps the real OBS editor while Show Control,
	 * Lights and Action are first-class product workspaces. */
	QPointer<QStackedWidget> pulsePages;
	QPointer<QWidget> pulseShowPage;
	QPointer<QWidget> pulseCameraPage;
	QPointer<QMainWindow> pulseCameraDockHost;
	QPointer<QWidget> pulseLightsHost;
	...
	QPointer<QWidget> pulseHorizontalPreviewCard;
	QPointer<QWidget> pulseStreamStatsCard;
	QPointer<QLabel> pulseStreamStatsTitle;
	QPointer<QLabel> pulseStreamStatsState;
	QPointer<QLabel> pulseStreamStatsUptime;
	QPointer<QLabel> pulseStreamStatsBitrate;
	QPointer<QLabel> pulseStreamStatsDropped;
	QPointer<QLabel> pulseStreamStatsSent;
	QPointer<PulseVerticalEditor> pulseVerticalEditor;
	QPointer<QStackedWidget> pulseCameraEditors;
	QPointer<QLabel> pulseEngineStatus;
	QPointer<QLabel> pulseVerticalStatus;
	QPointer<QComboBox> pulseHorizontalSceneSelector;
	QPointer<QComboBox> pulseVerticalSceneSelector;
	QPointer<QComboBox> pulseStageSelector;
	QPointer<QComboBox> pulseStageTransitionSelector;
	QPointer<QSpinBox> pulseStageTransitionDuration;
	QPointer<QComboBox> pulseVerticalStageTransitionSelector;
	QPointer<QSpinBox> pulseVerticalStageTransitionDuration;
	QPointer<QPushButton> pulseRepairVerticalButton;
	QPointer<QPushButton> pulseStreamButton;
	QPointer<QPushButton> pulseRecordButton;
	QPointer<QComboBox> pulseRecordingDestination;
	QPointer<QPushButton> pulseReplayButton;
	QPointer<QPushButton> pulseVirtualCameraButton;
	QPointer<QWidget> pulseCompactMixerBody;
	...
	QPointer<QLabel> pulseDestinationStatus;
	QPointer<QLabel> pulseChatStatus;
	QPointer<QComboBox> pulseChatProvider;
	QPointer<QLineEdit> pulseChatInput;
	...
	QString pulseHorizontalPreviewProvider;
	QString pulseVerticalPreviewProvider;
	QString pulseStreamStatsSelection;
	QHash<QString, quint64> pulseStreamStatsPreviousBytes;
	QHash<QString, qint64> pulseStreamStatsPreviousSampleMs;
	QHash<QString, double> pulseStreamStatsBitrateKbps;
	bool pulseStageUpdating = false;
	...
	void RestorePulseWeaverYouTubeAccount();
	void ShutdownPulseWeaverShell();
	void SetPulseWeaverWorkspace(int index);
	void UpdatePulseWeaverPreviewVisibility();
	void UpdatePulseWeaverShell();
	void RefreshPulseWeaverSceneSelectors();
	void RefreshPulseWeaverStages();
	void RefreshPulseWeaverTransitionSelector();
	void RefreshPulseWeaverPlatformPreviews();
	void CyclePulseWeaverPlatformPreview(bool vertical, int direction = 1);
	void RefreshPulseWeaverStreamStats();
	void CyclePulseWeaverStreamStats(int direction = 1);
	void ActivatePulseWeaverStage(int index);
	void ApplyPulseWeaverStage(int index, bool runTransitions);
	void FinalizePulseWeaverStageTransitions(quint64 serial, const QString &verticalScene,
					       const QHash<QString, QJsonObject> &routed);
	void ConfigurePulseWeaverAudioRouting(const QJsonObject &stage);
	void RestorePulseWeaverAudioRouting();
	void CapturePulseWeaverStage();
	void ManagePulseWeaverStages();
	void RefreshPulseWeaverCompactMixer();
	...
	void ApplyPulseWeaverLiveDestinationChange(const QString &provider, const QString &previousMode,
						       const QString &mode);
	void StartPulseWeaverSecondaryOutputs();
	void StopPulseWeaverSecondaryOutputs();
	...
	static void RenderPulseVertical(void *data, uint32_t cx, uint32_t cy);
...
public slots:
	QJsonObject PulseWeaverLumiaDestination(const QString &provider, bool start);
	void close();
...
#else
				QIcon trayIconFile = QIcon(":/res/images/pulseweaver.png");
#endif
