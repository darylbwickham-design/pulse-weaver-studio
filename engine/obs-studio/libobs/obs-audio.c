...
{
	if (!dup_src || obs_source_removed(dup_src) || !obs_source_active(dup_src))
		return false;
		    ...
		    struct audio_output_data *mixes)
{
	...
	/* ------------------------------------------------ */
	/* render audio data */
	/* Hold one source reference for this render pass. Shutdown can remove the
	 * capture concurrently, but cannot free it while these helpers use it. */
