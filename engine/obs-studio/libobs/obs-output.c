...
#include "util/pulse-output-time.h"
...
uint64_t obs_output_get_video_uptime_usec(obs_output_t *output)
{
	if (!obs_output_valid(output, "obs_output_get_video_uptime_usec") || !active(output))
		return 0;
	/* The interleaver normalizes every encoder to this output's own start.
	 * Reading its timestamps also avoids counting shared encoders' earlier
	 * recording time or treating 30 FPS tracks as if they were 60 FPS. */
	pthread_mutex_lock(&output->interleaved_mutex);
	const int64_t longest = pulse_longest_track_time_usec(output->highest_video_ts, MAX_OUTPUT_VIDEO_ENCODERS);
	pthread_mutex_unlock(&output->interleaved_mutex);
	return (uint64_t)longest;
}
